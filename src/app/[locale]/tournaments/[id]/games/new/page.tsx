  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/tournaments/${params.id}/games`)
      } else {
        const errorData = await response.json()
        alert(`Feil ved opprettelse av kamp: ${errorData.error || 'Ukjent feil'}`)
      }
    } catch (error) {
      ...
      <div className="px-4 py-6 sm:px-0">
        <div className="text-center">
          <div className="text-gray-500 text-lg mb-4">
            Du trenger minst 2 lag for å opprette kamper
          </div>
          <button
      ...
      <div className="max-w-md mx-auto">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Velg hjemmelag og bortelag for den nye kampen
          </p>
        </div>
